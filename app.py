...
QDRANT_COLLECTION_NAME = "audio_notes"
...
with add_tab:
    note_audio = audiorecorder(
        start_prompt="Nagraj notatkę audio",
    ...
    if note_audio:
        ...
        if st.button("Transkrybuj notatkę audio"):
            st.session_state["note_audio_text"] = transcribe_audio(st.session_state["note_audio_bytes"])

        if st.session_state["note_audio_text"]:
            st.session_state["note_text"] = st.text_area("Edytuj notatkę audio", value=st.session_state["note_audio_text"])
        
        if st.session_state["note_text"] and st.button("Zapisz notatkę audio", disabled=not st.session_state["note_text"]):
            add_note_to_db(note_text=st.session_state["note_text"])
            st.success("Notatka audio została zapisana!", icon="🎉")
